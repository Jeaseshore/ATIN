...
class welldataloader:
    ...
    def get_data(self):
        ...
        """
        self.date = self.dataset.data[:, 0]         # 日期
        return self.dataset
    ...
    def processing_sampling_deltat(self):   # 采样时间
    ...
    def forwards_data(self):
        ...
        length = len(data)
        num_features = len(feature_names)
        temp_forwards = np.zeros(num_features)
        forwards_forward = np.zeros((length, num_features))
        forwards_backward = np.zeros((length, num_features))

        for i in range(length):  # forward process
            row = data[i]
            forwards_forward[i] = temp_forwards
            for j in range(4, num_features - 1):
                if row[j] != 'nan':
                    temp = isFloat(row[j])
                    temp_forwards[j] = temp

        temp_forwards = np.zeros(num_features)
        for i in range(length - 1, -1, -1):  # backward process
            row = data[i]
            forwards_backward[i] = temp_forwards
            for j in range(4, num_features - 1):
                if row[j] != 'nan':
                    temp = isFloat(row[j])
                    temp_forwards[j] = temp

        self.forwards_forward = forwards_forward
        self.forwards_backward = forwards_backward
    ...
    def remove_columns(self):
        ...
        self.forwards_forward = np.delete(self.forwards_forward, delete_columns, axis=1)
        self.forwards_backward = np.delete(self.forwards_backward, delete_columns, axis=1)
    ...
    def make_TS_data(self, seqlength):
        ...
        date = self.date
        forwards_forward = self.forwards_forward
        forwards_backward = self.forwards_backward
        ...
        queLabel = deque(maxlen=seqlength)
        queDate = deque(maxlen=seqlength)
        for eachData, eachMask, eachdelta_f, eachdelta_b, eachDeltaF, eachDeltaB, eachLabel, eachff, eachfb, eachDate in \
                zip(data, mask, delta_t_forward, delta_t_backward, Deltas_f, Deltas_b, chosen_label, forwards_forward, forwards_backward, date):
            if sum(eachLabel) > -2:  # 选取存在采样（产液、含水）的时间点
                tempsample = [eachData, eachMask, eachdelta_f, eachdelta_b, eachff, eachfb]
                quedata.append(tempsample)
                queDelta.append([np.array([eachDeltaF]), np.array([eachDeltaB])])
                queLabel.append(eachLabel)
                queDate.append(getDateInterval(eachDate))
            else:
                continue
            if len(quedata) == seqlength:
                rnnData.append(list(quedata))
                rnnLabel.append(list(queLabel))
                rnnDelta.append(list(queDelta))
                rnnDate.append(list(queDate))
                Date.append(list(queDate))
        ...
        try:
            ...
            seqData = rnnData[:, :, 0, :]
            seqMask = rnnData[:, :, 1, :]
            seqdelta_f = rnnData[:, :, 2, :]
            seqdelat_b = rnnData[:, :, 3, :]
            seqforwards_f = rnnData[:, :, 4, :]
            seqforwards_b = rnnData[:, :, 5, :]
            seqDeltas_F = rnnDelta[:, :, 0]
            seqDeltas_B = rnnDelta[:, :, 1]
        ...
        return {'seqdata': seqData, 'seqmask': seqMask,
                ...
                'seqforwards_f': seqforwards_f, 'seqforwards_b': seqforwards_b,
...
class WellGroup:
    ...
    def __init__(self, dirname, seqlength=0):
        ...
        file_paths, filenames = load_filenames(dirname)
        for eachPath, eachFile in zip(file_paths, filenames):   # 所有生产井数据处于同一个文件夹
            tempPDP = welldataloader(dirname + eachFile)
...
if __name__ == '__main__':
    ...
    wellgroup = WellGroup('../../../data/wells/', seqlength=48)
    dataset = wellgroup.get_ts_wells()
    np.save('../../tempdata/wells_data_mask_onlymeasure_forwards48.npy', dataset)  # 注意带上后缀名
    pass
